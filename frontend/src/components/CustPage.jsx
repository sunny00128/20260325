...
export default function CustPage({ onBack }) {
  ...
  const fetchData = useCallback(async (kw = '') => {
    setLoading(true);
    try {
      const res = await api.get('/api/cust', { params: kw ? { search: kw } : {} });
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`確定要刪除客戶 ${id} 嗎？`)) return;
    try {
      await api.delete(`/api/cust/${id}`);
      showMsg('刪除成功');
      fetchData(search);
    } catch (err) {
  ...
  const handleSave = async () => {
    if (!form.cust_code.trim() || !form.cust_name.trim()) {
      showMsg('客戶代碼與名稱為必填欄位', 'error');
      return;
    }
    setSaving(true);
    try {
      if (mode === 'add') {
        await api.post('/api/cust', form);
        showMsg('新增成功');
      } else {
        await api.put(`/api/cust/${form.cust_code}`, form);
        showMsg('修改成功');
      }
      setShowModal(false);
      fetchData(search);
          ...
          <table className="table table-hover mb-0">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={row.cust_code}>
                  <td><span className="badge-code">{row.cust_code}</span></td>
                  <td>{row.cust_name}</td>
                  <td>{row.remark || '-'}</td>
                  <td className="text-center">
                    ...
                    <button
                      ...
                      onClick={() => handleDelete(row.cust_code)}
                    >
      ...
      {showModal && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal-box">
            ...
            <div className="mb-3">
              <label className="form-label fw-semibold">客戶代碼 <span className="text-danger">*</span></label>
              <input
                type="text"
                className="form-control"
            ...
            <div className="mb-3">
              <label className="form-label fw-semibold">客戶名稱 <span className="text-danger">*</span></label>
              <input
                type="text"
                className="form-control"
